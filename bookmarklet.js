// Welcome to the worst pile of random text put together into
// a program that somehow manages to start up and work.
// this is certified shitcode, and i am not responsible for
// all of it
...
// Look into bundling jquery with only the parts we need
...
function run() {
    ...
    var theme = '#da42f5';
    // used but basically useless
    var headerTheme = '#da42f5';
    ...
    const Main = newElement('div', document.body, {id: 'main-1234-1234'});
    const MainContainer = newElement('div', document.body, {id: 'main-1234-1234-c'});

    const header = newElement('h1', Main, {id: 'header-1234-1234'});
    header.innerHTML = `&nbsp;&nbsp;${name} [2.0] (beta)`;
    const subheader = newElement('h1', Main, {id: 'header2-1234-1234'})
    subheader.innerHTML = '&nbsp;&nbsp;&nbsp; | Right click to open menu | BACKSLASH to hide |'
    const logoImage = newElement('img', Main, {src: 'https://camo.githubusercontent.', id: 'logo-1234-1234'})
...
@import url('https://fonts.googleapis.com/css2?family=Nunito&display=swap');
    ...
    * {
    ...
    .button-c-1234-1234:last-child button {
        border-radius: 0 0 5px 5px;
    }
...
`
    ...
    class UISection {
        ...
        constructor(id, name, x, y) {
            this._initializeProperties(id, name, x, y);
            this._setupDraggable();
            this._setupHeaderMouseDownEvent();
        }

        _initializeProperties(id, name, x, y) {
            this.div = newElement('div', Main,{ className: 'section-1234-1234'});
            this.div.id = `${name}-1234-1234`;
            this.header = newElement('div', this.div, { className: 'header-1234-1234' });
            this.header.innerHTML = name;
            this.header.addEventListener('contextmenu', e => e.preventDefault());
            this.buttonsDiv = newElement('div', this.div, { className: 'buttons-1234-1234' });
        ...
        _toggleSection() {
            this.header.style.borderRadius = this.enabled ? '5px 5px 0 0' : '5px';
            this.buttonsDiv.style.transform = this.enabled ? 'scaleY(1)' : 'scaleY(0)';
        }
        ...
        _createButtonContainer() {
            return newElement('div', this.buttonsDiv, { className: 'button-c-1234-1234' });
        }

        _createButton(container, name) {
            const button = newElement('button', container, { className: 'button-1234-1234' });
        ...
        _addOptionsIcon(button) {
            button.innerHTML += '<svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512"><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><style>svg{fill:#ffffff}</style><path d="M0 96C0 78.3 14.3 64 32 64H416c17.7 0 32 14.3 32 32s-14.3 32-32 32H32C14.3 128 0 113.7 0 96zM0 256c0-17.7 14.3-32 32-32H416c17.7 0 32 14.3 32 32s-14.3 32-32 32H32c-17.7 0-32-14.3-32-32zM448 416c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32s14.3-32 32-32H416c17.7 0 32 14.3 32 32z"/></svg>'; // SVG data truncated for brevity
            button.addEventListener('contextmenu', e => e.preventDefault());
        }

        _createOptionsMenu(container, options) {
            const menu = newElement('div', container, { className: 'menu-1234-1234' });
        ...
        _addCheckboxOption(menu, optionData) {
            const option = newElement('div', menu, { className: 'option-1234-1234' });
            option.innerHTML = optionData.name;
            const checkbox = newElement('input', option, { className: 'checkbox-1234-1234' });
        ...
        _addRadioOption(menu, optionData) {
            const option = newElement('div', menu, { className: 'option-1234-1234' });
            option.innerHTML = optionData.name;
            const fieldset = newElement('fieldset', option, { className: 'fieldset-1234-1234' });
            optionData.values.forEach(value => {
                const div = newElement('div', fieldset, { className: 'div-1234-1234' });
                const input = newElement('input', div, { className: 'input-radio-1234-1234'});
                input.type = 'radio';
                input.name = optionData.name;
                input.value = value;
                input.checked = value === optionData.checked;
                const label = newElement('label', div, { className: 'label-1234-1234' });
        ...
        _addTextOption(menu, optionData) {
            const option = newElement('div', menu, { className: 'option-1234-1234' });
            option.innerHTML = optionData.name;
            const input = newElement('input', option, { className: 'input-text-1234-1234' });
            input.type = 'text';
        ...
        addSeparator() {
            let separator = newElement('div', this.buttonsDiv, { className: 'separator-1234-1234' });
        }
    ...
    * {
    ...
    function loadSettingsFromLocalStorage(active, values) {
        ...
        for (let i = 0; i < sectionInstances.length; i++) {
            for (let j = 0; j < settings[sectionInstances[i].id].active[0].length; j++) {
                ...
                if (values != null) {sectionInstances[i]._setButtonValue(j, values)}
                if (state) {sectionInstances[i]._setButtonStateWithIndex(j, state)}
    ...
    function createFramebar(title) {
        const framebar = newElement('div', false, { innerHTML: title, className: 'framebar-1234-1234' });
        const minimize = newElement('button', framebar, { innerHTML: '–', className: 'minimize-1234-1234'});
        const fullscreen = newElement('button', framebar, { innerHTML: '□', className: 'fullscreen-1234-1234' });
        const close = newElement('button', framebar, { innerHTML: '✕', className: 'close-1234-1234' });
    
        framebar.appendChild(fullscreen);
        framebar.appendChild(close);
        framebar.appendChild(minimize);
    
        return { framebar, minimize, fullscreen, close };
    }
    
    function ScratchPlus(active, values) {
        if (active) {
            const scratchLoader = newElement('div', false, { id: 'scratch-loader-1234-1234', style: { height: '60%', width: '50%' }, className: 'frame-1234-1234'});
    
            const { framebar, minimize } = createFramebar('Scratch Loader+');
    ...
    function websiteLoader(active, values) {
        const websiteLoader = newElement('div', false, { id: 'website-loader-1234-1234', style: { height: '65%', width: '60%' }, className: 'frame-1234-1234'});
    
        const { framebar, minimize, fullscreen, close } = createFramebar('Website Loader');
        ...
        fullscreen.addEventListener('click', () => {
            if (websiteLoader.style.height !== '100%') {
                ...
                $(websiteLoader).position({ my: "center", at: "center", of: window });
            } else {
    ...
    const Test = new UISection('uitests','ui stuff', 25, 50)
    ...
    document.addEventListener('keydown', e => {
        if (e.keyCode == 220) {
            ...
            Main.style.animation = disabled ? 'fade-in 3s cubic-bezier(0.075, 0.82, 0.165, 1)' : 'none'
